...
module.exports = {
  ...
  run: async ({ client, message, args }) => {
    ...
    if(Object.keys(filters).includes(args[0])) {
      const filter = await client.distube.setFilter(message, args[0] === 'clear' ? false : args[0]);
      if (args[0] === 'clear') {
        await client.distube.setFilter(message, ['customspeed']);
      }
      message.channel.send(formatMsg(`Current queue filter: \`${filter.filter(v => v !== 'customspeed').join(", ") || "Off"}\``)).catch(console.error);
    } else {
        getEmbedMsg(message, "#ff0000", `Type \`${CONFIG.prefix}filter <Filter name>\` to toggle a filter`, `**__Please choose these options__**\n\`${Object.keys(filters).filter(v => v !== 'customspeed').join(', ')}\``);
    }
