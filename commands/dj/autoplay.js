...
module.exports = {
  ...
  run: async ({ client, message, args }) => {
    ...
    if (args.length > 0) {
      return message.channel.send(formatMsg(`Type \`${CONFIG.prefix}autoplay\` to toggle autoplay queue`)).catch(console.error);
    }
    ...
    const mode = await client.distube.toggleAutoplay(queue);
    message.channel.send(formatMsg(`Set autoplay mode to \`${mode ? "On" : "Off"}\``)).catch(console.error);
  }
}