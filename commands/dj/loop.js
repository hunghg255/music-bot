  ...
  run: async ({ client, message, args }) => {
    ...
    if (args.length > 1) {
      getEmbedMsg(
        message,
        '#ff0000',
        `Type \`${CONFIG.prefix}loop <Repeat Options>\``,
        `**Please use a number between 0 and 2**\n\n**0**: \`Disable Repeat\`\n**1**: \`Repeat current song\`\n**2**: \`Repeat queue\``
      );
      return;
    }
    ...
    if (0 <= Number(args[0]) && Number(args[0]) <= 2) {
      ...
      getEmbedMsg(
        ...
        '#00ff00',
        ' ',
        `**Repeat mode set: ${args[0]
          .replace('0', `\`Disbale Repeat\``)
          .replace('1', `\`Repeat current song\``)
