const { getEmbedMsg, getStatus, addReact, isPermsDJ, formatMsg, CONFIG } = require('../../utils/index.js');
...
module.exports = {
  ...
  run: async ({client, message, args}) => {
    ...
    if (args.length > 0) {
      return message.channel.send(formatMsg(`Type \`${CONFIG.prefix}pause\` to pause current song`)).catch(console.error);
    }
