...
module.exports = {
  ...
  run: async ({ client, message, args }) => {
    ...
    if (args.length > 0) {
      return message.channel.send(formatMsg(`Type \`${CONFIG.prefix}lc\` to removes absent user’s songs from the Queue.`));
    }
    ...
    message.channel.send(formatMsg(`Cleared \`${length}\` songs`));
  }
}