...
module.exports = {
  ...
  run: async ({client, message, args}) => {
    ...
    if (args.length > 1) {
      return message.channel.send(formatMsg(`Type \`${CONFIG.prefix}seek <Number seconds>\` to seek current song`)).catch(console.error);
    }
    ...
    if (!!Number(args[0]) || Number(args[0]) == 0) {
      if (Number(args[0]) >= 0) {
        await client.distube.seek(message, Number(args[0]));
        message.channel.send(formatMsg(`Seeked this song for \`${args[0]} seconds\` done`)).catch(console.error);
      } else {
        message.channel.send(formatMsg(`Must be bigger or equal to 0`)).catch(console.error);
      }
    } else {
      message.channel.send(formatMsg(`Type \`${CONFIG.prefix}seek <Number seconds>\``)).catch(console.error);
    }
  }
}