  ...
  run: async ({ client, message, args }) => {
    ...
    if (!args.length || args.length > 1) {
      getEmbedMsg(
        message,
        '#ff0000',
        `Please add a Speed Amount between 0+ and 2!`,
    ...
    if (!!parseFloat(speedValue)) {
      if (parseFloat(speedValue) > 0 && parseFloat(speedValue) <= 2) {
        ...
        ].customspeed = `atempo=${parseFloat(speedValue)}`;
        client.distube.filters =
          client.filters[`${message.guild.id}-${client.user.id}`];
        await client.distube.setFilter(message, ['customspeed']);
        message.channel
          .send(formatMsg(`Changed speed to \`${speedValue}x\``))
          .catch(console.error);
      } else {
        getEmbedMsg(
          message,
