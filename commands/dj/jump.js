...
module.exports = {
  ...
  run: async ({ client, message, args }) => {
    ...
    if (args.length > 1) {
      return message.channel.send(formatMsg(`Type \`${CONFIG.prefix}jump <Position song>\` to jump into a song`)).catch(console.error);
    }
    ...
    if(0 < Number(args[0]) && Number(args[0]) < queue.songs.length){
      client.distube.jump(message, parseInt(args[0]))
        .catch(err => {
           message.channel.send(formatMsg("Invalid song number.")).catch(console.error);
        });
      message.channel.send(formatMsg(`Jumped ${parseInt(args[0])} songs!`)).catch(console.error);
    } else {
      message.channel.send(formatMsg("Invalid song number.")).catch(console.error);
    }
  }
