...
module.exports = {
  ...
  run: async ({client, message, args}) => {
    ...
    if (!args.length) {
      return message.channel.send(formatMsg(`Type \`${CONFIG.prefix}rm <Position song>\` to remove a song`));
    }
    ...
    if (!isDj && args.length > 1) {
      return message.channel.send(formatMsg(`You need to a \`DJ\` to remove more than 1 song`));
    }
    ...
    if (!isDj && args.length == 1) {
      ...
      if (songRemove.user.id !== authorId) {
        return message.channel.send(formatMsg(`You can't remove other people's songs`));
      }
