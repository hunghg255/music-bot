...
module.exports = {
  ...
  run: async ({ client, message, args }) => {
      ...
      if (args.length > 0) {
        return message.channel.send(formatMsg(`Type \`${CONFIG.prefix}join\` to add bot into VC`)).catch(console.error);
      }
      
      try {
        await client.distube.voices.join(message.member.voice.channel);
        message.channel.send(formatMsg('It is me pleasure to serve you <3')).catch(console.error);
      } catch (err) {
        const embed = new Discord.MessageEmbed()
          .setColor('#ff0000')
          .setTitle(' ')
          .setDescription(`**I do not have permission to join this voice channel**`)
          .setTimestamp();
