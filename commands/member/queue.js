...
module.exports = {
  ...
  run: async ({client, message, args}) => {
    ...
    if (args.length > 0) {
      return message.channel.send(formatMsg(`Type \`${CONFIG.prefix}q\` to show queue`)).catch(console.error);
    }
    addReact(message);

    let queue = client.distube.getQueue(message);
    if (!queue) {
      return message.channel.send(formatMsg(`Queue Empty!!`)).catch(console.error);
    }
