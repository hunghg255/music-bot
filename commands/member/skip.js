...
module.exports = {
  ...
  run: async ({client, message, args}) => {
    ...
    if (args.length > 0) {
      return message.channel.send(formatMsg(`Type \`${CONFIG.prefix}skip\` to vote skip current song`)).catch(console.error);
    }
    ...
    if (client[`skip-${serverId}`].usersId.includes(author.id)) {  
      return message.channel.send(formatMsg(`You voted skip before that.`)).catch(console.error);
    }
