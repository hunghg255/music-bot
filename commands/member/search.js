const { getEmbedMsg, addReact, formatMsg, removeSpoiler, CONFIG } = require('../../utils/index.js');
...
module.exports = {
  ...
  run: async ({client, message, args}) => {
    ...
    if (!args.length) {
      return message.channel.send(formatMsg(`Type \`${CONFIG.prefix}search <Song name>\` to search a song`)).catch(console.error);
    }
    ...
    if (!client[`time-${message.author.id}`]) client[`time-${message.author.id}`] = t;
  }
}