...
module.exports = {
  ...
  run: async ({ client, message, args }) => {
    ...
    if (args.length > 0) {
      return message.channel.send(formatMsg(`Type \`${CONFIG.prefix}dc\` to disconect with bot`)).catch(console.error);
    }
    
    try {
      await client.distube.voices.leave(message);
      message.channel.send(formatMsg(`**Ok If you have to go say goodbye. Love ya 3000**`)).catch(console.error);
    } catch (err) {}
  }
}