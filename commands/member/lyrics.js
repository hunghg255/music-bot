...
module.exports = {
  ...
  run: async ({ client, message, args }) => {
    ...
    if (args.length > 0) {
      try {
        message.channel.send(formatMsg(`Searching lyrics: \`${args.join(' ')}\``)).catch(console.error);
        const result = await getLyric(args.join(' '));
   
        if (!result) {
          return message.channel.send(formatMsg(`Could not find lyrics!!`)).catch(console.error);
        }
    ...
    if (queue.songs.length) {
      try {
        message.channel.send(formatMsg(`Searching lyrics: \`${queue.songs[0].name}\``)).catch(console.error);
